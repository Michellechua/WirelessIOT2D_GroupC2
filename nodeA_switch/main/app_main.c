...
void app_main()
{
    ...
    status_param = esp_rmaker_param_create("Status",
        "esp.param.status", esp_rmaker_str("Starting..."), PROP_FLAG_READ);
    ...
    // sensor_device = esp_rmaker_device_create("sensor_update", ESP_RMAKER_DEVICE_TEMP_SENSOR, NULL);

    // // Create a parameter for temperature
    // temp_param = esp_rmaker_param_create("Temperature", 
    //     "esp.param.temperature",  // standard parameter type
    //     esp_rmaker_float(25.0),
    //     PROP_FLAG_READ
    // );  // properties


    // // Add bounds (min, max, step)
    // esp_rmaker_param_add_bounds(temp_param, esp_rmaker_float(0.0), esp_rmaker_float(100.0), esp_rmaker_float(0.1));
    ...
    // // Assign as the primary parameter
    // esp_rmaker_device_assign_primary_param(sensor_device, status_param);

    // // Add the device to the node
    // esp_rmaker_node_add_device(node, sensor_device);

    // // To update the temperature value (you would call this in your main loop or timer callback)
    // new_temperature = 27.5; // Your actual sensor reading or value
    // esp_rmaker_param_update_and_report(temp_param, esp_rmaker_float(new_temperature));


    // esp_rmaker_device_add_param(sensor_device, status_param);
    ...
    //MultiThread, runs separat loop
    start_temp_update_task();
    // // ESP_LOGE(TAG, "start temp update task here...");
}
